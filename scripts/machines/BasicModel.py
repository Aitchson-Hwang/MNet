...
class BasicModel(object):
    def __init__(self, datasets =(None,None), models = None, args = None, **kwargs):
        ...
        if self.args.arch == 'mssnet':
            self.model = archs.__dict__[self.args.arch]()
        elif self.args.arch == 'locatenet':
            self.model = archs.__dict__[self.args.arch]()
        elif self.args.arch == 'vvv4n':
            self.model = archs.__dict__[self.args.arch]()
        else:
        ...
        if self.args.arch2 != 'None':
            ...
            self.model2 = archs.__dict__[self.args.arch2](args=args).to('cuda')
            self.optimizer2 = torch.optim.Adam(filter(lambda p: p.requires_grad, self.model2.parameters()),
                                               lr=args.lr,
                                               betas=(args.beta1, args.beta2),
                                               weight_decay=args.weight_decay)

            self.title2 = '_' + args.machine + '_' + args.data + '_' + args.arch2
            self.args.checkpoint2 = os.path.join(args.checkpoint2, self.title2)
            if not isdir(self.args.checkpoint2):
                mkdir_p(self.args.checkpoint2)
            print("==> creating model2 [Finish]")
        ...
        if self.args.withbest == 0:
            self.best_acc = 0
        else:
            self.best_acc = self.args.withbest
        self.best_acc2 = 0
        self.best_acc3 = 0
        self.is_best = False
        ...
        # flops, params = profile(self.model, inputs=(self.norm(input_tensor),), verbose=False)
        ...
        # else:
        #     print('==> Model1 params: %.2fM' % (sum(p.numel() for p in self.model.parameters()) / 1000000.0))
        #     print('==> Model2 params: %.2fM' % (sum(p.numel() for p in self.model2.parameters()) / 1000000.0))
        #     print('==> Total devices: %d' % (torch.cuda.device_count()))
        #     print('==> Current Checkpoint: %s' % (self.args.checkpoint))
        #     print('==> Current Checkpoint2: %s' % (self.args.checkpoint2))
